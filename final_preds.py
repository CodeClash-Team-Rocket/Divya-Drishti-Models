...
class Application:
    ...
    def __init__(self):
        ...
        self.T.config(
            text="Sign Language To Text Conversion", font=("Courier", 30, "bold")
        )
        ...
        self.speak.config(
            text="Speak", font=("Courier", 20), wraplength=100, command=self.speak_fun
        )

        self.clear = tk.Button(self.root)
        self.clear.place(x=1205, y=630)
        self.clear.config(
            text="Clear", font=("Courier", 20), wraplength=100, command=self.clear_fun
        )
    ...
    def video_loop(self):
        try:
            ...
            if cv2image.any:
                ...
                self.imgtk = (
                    imgtk  # Store the image reference to prevent garbage collection
                )
                    ...
                    if image.all:
                        ...
                        if handz[0]:
                            ...
                            for t in range(0, 4, 1):
                                cv2.line(
                                    white,
                                    (self.pts[t][0] + os, self.pts[t][1] + os1),
                            ...
                            cv2.line(
                                white,
                                (self.pts[0][0] + os, self.pts[0][1] + os1),
                                (self.pts[5][0] + os, self.pts[5][1] + os1),
                                (0, 255, 0),
                                3,
                            )
                            ...
                            for i in range(21):
                                cv2.circle(
                                    white,
                                    (self.pts[i][0] + os, self.pts[i][1] + os1),
                                    2,
                                    (0, 0, 255),
                                    1,
                                )
                            ...
                            self.imgtk2 = imgtk  # Store the image reference to prevent garbage collection
                            self.panel2.config(image=imgtk)

                            self.panel3.config(
                                text=self.current_symbol, font=("Courier", 30)
                            )
    ...
    def predict(self, test_image):
        ...
        if ch1 == 1:
            ...
            if (
                (
                    self.distance(self.pts[8], self.pts[12])
                    - self.distance(self.pts[6], self.pts[10])
                )
                < 8
            ) and (
                self.pts[6][1] > self.pts[8][1]
